...
function plot_eigen(eigenvalues, gap::Nothing, E_exciton::Nothing, flag::AbstractString, type_of_calc::AbstractString)
    current_path = pwd()
    fig = Figure()
    ax = Axis(fig[1,1], title = "Eigenvalues", xlabel = "index of eigs", ylabel = "Energy [au]")
    indexes = [i_ for i_ in eachindex(eigenvalues)]
...
function plot_norm(ts::AbstractArray, norm::AbstractArray, type_of_calc::AbstractString)
    ...
    fig = Figure()
    ax = Axis(
        fig[1,1], title = "Norm", xlabel = L"$t$ [au]", ylabel = L"$\langle \Psi(t)| \Psi(t)\rangle $",
        limits = (minimum(ts), maximum(ts), 0.75, 1.25)
        )
...
function plot_groundstate(ts::AbstractArray, norm::AbstractArray, type_of_calc::AbstractString)
    ...
    fig = Figure()
    ax = Axis(
        fig[1,1], title = "Norm", xlabel = L"$t$ [au]", ylabel = L"$\langle \Psi_0| \Psi(t)\rangle $", 
        limits = (minimum(ts), maximum(ts), 0.75, 1.25)
    )
